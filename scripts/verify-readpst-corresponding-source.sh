...
reference_pairs = [
    ...
    ("scripts/macos-dylib-validation.sh", "scripts/macos-dylib-validation.sh"),
    ("scripts/test-macos-dylib-validation.sh", "scripts/test-macos-dylib-validation.sh"),
...
for relative in executable_scripts:
    if not (companion / relative).stat().st_mode & 0o111:
        fail(f"required companion script is not executable: {relative}")
passed("companion executable scripts preserve executable permissions")
