...
or `/System/Library/`; Homebrew, MacPorts, user-specific, relative, and loader
token paths are rejected. The repository verifier parses dependency records one
architecture at a time through the exact companion copy of
`scripts/macos-dylib-validation.sh`, rather than treating universal Mach-O
header lines as install names. Run its parser regression checks from the
companion root with:

```sh
bash scripts/test-macos-dylib-validation.sh
```
